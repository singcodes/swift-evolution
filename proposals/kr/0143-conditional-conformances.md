...
Conditional conformances 는 제너릭타입이 타입 인수가 지정한 요구 사항과 만날 때만 특정 프로토콜을  따르는 것을 말합니다. 예를 들어, `Array`컬렉션은 아래에 표현한 것처럼 그 요소가 `Equatable`일 때만, `Equatable`을 구현(implement)할 수 있습니다.:
